        ...
        try:
            while True:
                ...
                for i, (name, data) in enumerate(totals.items()):
                    ...
                    if(float(data['kbps_total'])/1024 > 10):
                        if(name not in programs_sent_note):
                            command = f'notify-send "{name}" "Is using to much data: {int(float(data['kbps_total'])/1024)} MB"'
                            os.system(command)
                            programs_sent_note.append(name)
